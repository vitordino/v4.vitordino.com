...
ogImage: '/images/declarative-styles/og.png'
twitterImage: '/images/declarative-styles/twitter.png'
tags:
  - web
  - css
  - react
...
import TextSlice from '@components/TextSlice.astro'
import QuoteSlice from '@components/QuoteSlice.astro'
import ImageSlice from '@components/ImageSlice.astro'

import vertbaseImage from './vertbase.png'
...
<QuoteSlice author="— Me, just a couple of hours after learning how to apply background-color to a div">
...
For example, a coworker said he was struggling to build a grid layout with the current tools (_flex-box_ had just launched), and every css library limited him on having a single fixed gutter width — percentage or pixels — on every breakpoint.
...
## Fast forward some years, and we have landed into the train of **JS**: Single Page Applications now reign and, of course, _React_ and _CSS-in-JS_ libraries came along.
...
It’s now easier than ever write our own _design systems_, and have a direct way to cross the lines between styles and markup when we need, and write our own rules and themes with JS functions at run time, and not only compiled _SASS_ or _CSS_ variables. And all of this is awesome — and I use it every day including at this website.
...
If we take a look on the other side of the ring we can clearly see another tribe being formed: the _prop junkies_. They are the exact opposite of the last example, and they propagate the message that every layout can be written with less than a dozen components filled with style-related props. They are descendants of _utility-first CSS_ tribes — [tailwind](https://tailwindcss.com/) for example — and their most famous prophets are [rebass](https://rebassjs.org/) and [styled-system](https://styled-system.com/).
...
<ImageSlice>
	<Image alt='vertbase.com' src={vertbaseImage} />
	<div slot='caption'>
		<a href='https://vertbase.com'>vertbase.com</a> project, where I worked with{' '}
		<a href='https://significa.co'>significa</a>, check out their{' '}
		<a href='https://significa.co/showcase/vertbase'>case page</a>
	</div>
</ImageSlice>
...
As mentioned above, we can cross the _theme_ and _props_ to build a readable component. As one can imagine, I began just using this technique for grid components, but found use for it on many scenarios, on different projects.
...
After working with some projects adopting this pattern, I’ve built a library called [_etymos_](https://github.com/vitordino/etymos) — it’s currently based on `styled-components` — although I’m planning on releasing an agnostic or [_emotion_](http://emotion.sh) version too — and really [lightweight (1.2kb)](https://bundlephobia.com/result?p=etymos).
...
Other examples can be seen for this exact site for [_Grid_](https://github.com/vitordino/vitordino.com/blob/master/src/components/Grid/index.js), [_Text_](https://github.com/vitordino/vitordino.com/blob/master/src/components/Text/index.js) and [_Spacer_](https://github.com/vitordino/vitordino.com/blob/master/src/components/Spacer/index.js) components.
...
Any doubts, questions, or further ideas please open a [_issue_](https://github.com/vitordino/etymos/issues) or a [_pull request_](https://github.com/vitordino/etymos/issues) on github.
...
- [_A conceptual look at theming_](https://jxnblk.com/blog/a-conceptual-look-at-theming/) and [_Themeability_](https://jxnblk.com/blog/themeability/) by [Brent Jackson](https://jxnblk.com/)

- [_Why I Write CSS in JavaScript_](https://mxstbr.com/thoughts/css-in-js) and [_Margin considered harmful_](https://mxstbr.com/thoughts/margin) by [Max Stoiber](https://mxstbr.com)

- [_Declarative Design Tools_](https://jon.gold/2016/06/declarative-design-tools/) by [Jon Gold](https://jon.gold)

- [_Styling themes_](https://johno.com/styling-themes/) by [John Otander](https://johno.com)

- [_Component Styling API_](http://mrmrs.cc/writing/component-styling-api/) by [Adam Morse](http://mrmrs.cc/)

- [_Simplifying layouts with Stacks_](https://souporserious.com/easier-layouts-with-stacks/) by [Travis Arnold](https://souporserious.com)

- [_There Is No Design System_](https://24ways.org/2019/there-is-no-design-system/) by [Jina Anne](https://www.sushiandrobots.com/)
